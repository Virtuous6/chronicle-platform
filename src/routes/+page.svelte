  ...
  <main class="mx-auto max-w-7xl px-6 py-12">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <aside class="lg:col-span-3">
        <h2 class="text-xs font-semibold text-chronicle-text-muted uppercase tracking-wider mb-6">Notes</h2>
        <div class="space-y-6">
          {#each featuredNotes as article}
            <a href="/{article.slug}" class="block group">
              <article class="border border-chronicle-teal bg-chronicle-bg-card p-4 rounded-sm hover:bg-chronicle-teal-light transition-colors">
                ...
                <h3 class="text-lg font-serif font-bold text-chronicle-text group-hover:text-white mb-2">
                  {article.title}
                </h3>
                <p class="text-sm text-chronicle-text-light line-clamp-2">{article.excerpt}</p>
              </article>
            </a>
          {/each}
        </div>
      ...
      <section class="lg:col-span-6">
        <h2 class="text-xs font-semibold text-chronicle-text-muted uppercase tracking-wider mb-6">News</h2>
        
        <!-- Hero Featured Article -->
        {#if newsArticles.length > 0}
          <a href="/{newsArticles[0].slug}" class="block group mb-8">
            <article class="relative overflow-hidden rounded-sm bg-chronicle-teal text-white">
        ...
        <div class="space-y-6">
          {#each newsArticles as article}
            <a href="/{article.slug}" class="block group">
              <article class="border-b border-chronicle-teal pb-6">
                <div class="text-xs text-chronicle-text-muted uppercase mb-2">
                  {formatDate(article.published_at)}
                </div>
                <h3 class="text-xl font-serif font-bold text-chronicle-text group-hover:text-white mb-2">
                  {article.title}
                </h3>
                <p class="text-sm text-chronicle-text-light">{article.excerpt}</p>
              </article>
            </a>
          {/each}
        </div>
      ...
      <!-- Right Column - Videos -->
      <aside class="lg:col-span-3">
        <h2 class="text-xs font-semibold text-chronicle-text-muted uppercase tracking-wider mb-6">Videos</h2>
        <div class="space-y-6">
          {#each videos as video}
            <a href="/{video.slug}" class="block group">
              <article class="border border-chronicle-teal bg-chronicle-bg-card rounded-sm overflow-hidden hover:bg-chronicle-teal-light transition-colors">
                {#if video.hero_image_url}
                  <img src={video.hero_image_url} alt={video.title} class="w-full h-32 object-cover" />
                {/if}
                <div class="p-4">
                  <h3 class="text-sm font-serif font-bold text-chronicle-text group-hover:text-white mb-1">
                    {video.title}
                  </h3>
