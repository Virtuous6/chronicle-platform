...
<div class="min-h-screen bg-stone-100">
  ...
  {#if article.hero_image_url}
    <div class="relative h-96 overflow-hidden bg-stone-800">
      <img 
  ...
  {:else}
    <div class="h-64 bg-gradient-to-br from-blue-600 to-purple-700"></div>
  {/if}
  ...
  <article class="mx-auto max-w-3xl px-6 -mt-32 relative z-10">
    ...
    <div class="bg-white rounded-lg shadow-xl p-8 mb-8">
      <div class="text-xs text-stone-500 uppercase tracking-wider mb-4">
        <a href="/" class="hover:text-stone-900">Chronicle</a>
      ...
      {#if article.author}
        <div class="flex items-center gap-3 mb-4">
          ...
          {/if}
          <div>
            <div class="font-medium text-stone-900">{article.author.name}</div>
            <div class="text-sm text-stone-500">{formatDate(article.published_at)}</div>
          </div>
        </div>
      {/if}
...
<style>
  :global(.prose h1) {
    @apply text-3xl font-bold text-stone-900 mt-8 mb-4;
  }

  :global(.prose h2) {
    @apply text-2xl font-bold text-stone-900 mt-6 mb-3;
  }

  :global(.prose h3) {
    @apply text-xl font-bold text-stone-900 mt-4 mb-2;
  }
  ...
  :global(.prose code) {
    @apply bg-stone-100 px-2 py-1 rounded text-sm font-mono text-stone-800;
  }

  :global(.prose pre) {
    @apply bg-stone-900 text-stone-100 p-4 rounded-lg overflow-x-auto my-4;
  }
